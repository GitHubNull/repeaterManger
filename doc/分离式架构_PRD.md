...
| 越权测试 | 多用户会话管理、方案配置、6 种字段类型、字段自动替换、AND/OR/NOT 条件组合判决、5 种目标 x 9 种方法的判断规则、Scope 自动拦截、API 去重引擎、4 种相似度算法、实时进度展示 |
...
- **用户会话 (UserSession)**: 一个越权测试身份，包含 字段值、关联的方案、超时/重试/并发等重放参数
...
**会话管理**:
- 用户会话（UserSession）的完整 CRUD，每个会话绑定独立的 字段值和 方案
- 支持从剪贴板或请求报文中智能解析会话信息（自动提取 字段值）
- 支持会话启用/禁用、颜色标记

**方案与字段管理**:
- 方案（Scheme）：定义一组字段集合，对应某个认证体系的凭据分布
- 字段（FieldDefinition）：支持 6 种位置类型 — HTTP Header、JSON Body、XML Body、表单字段、Multipart 字段、URL 参数
- 字段替换引擎：根据 字段配置自动替换报文中的身份凭据；支持 null 值（删除 Token）
- 方案和字段均支持全局持久化（跨项目复用）
