...
| 4 | **中等** | 令牌位置列表为空时静默返回原始请求 | `TokenReplacementEngine.java` | ⚠️ 部分修复（调用方已加警告） |
...
| 4 | 令牌位置为空静默返回 | ⚠️ 部分修复 | 调用方(ReplayEngine/AutoTestEngine)已加警告；但 `TokenReplacementEngine` 内部仍静默 |
...
## Bug 4（中等）：令牌位置列表为空时静默返回原始请求

### 涉及文件

`src/main/java/org/oxff/repeater/privilege/TokenReplacementEngine.java` 第 52-58 行

### 问题描述

当某个用户会话关联的方案没有令牌位置（`locations` 为空列表），`replaceTokens` 方法直接返回原始请求字节数组，不做任何令牌替换，也不产生任何警告日志。
...
- 如果测试用户的方案配置错误（未关联任何令牌位置），该用户将使用**代理捕获到的原始用户的令牌**发送请求
- 这完全失去了越权测试的意义：测试用户使用了别人的有效令牌，所有请求都会通过鉴权，响应与基线一致 → 相似度 ≈ 1.0 → 被误判为 ESCALATED
- 没有任何日志警告提示用户配置问题

### 根因

`replaceTokens` 作为无状态工具方法，对输入校验过于宽松。将"空位置列表"与"空请求"同等对待是不合理的——空位置列表是一个配置错误信号，应报告给调用方。
...
**建议修复顺序**：
1. **第一优先**：Bug 4（TokenReplacementEngine 沉默返回 → 显式警告）
