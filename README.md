...
> **当前版本**: v2.2.0 | **最低要求**: Burp Suite Professional 2024+ (Montoya 扩展 API) + Java 17+
...
2. 添加全局规则（共享）或项目规则（独立），配置提取源和提取方法
3. 通过右键菜单或自动触发提取，插件会自动从不规则请求中提取标准 API 路径
4. 提取结果可在请求列表中查看和管理

### 越权测试

1. 在配置面板中设置用户会话（含凭证/Token）和 Token 位置
2. 配置判断规则（如何判定越权成功）
3. 设置请求范围（哪些 URL 需要测试）
4. 开启自动检测：插件拦截匹配范围的代理流量，自动替换 Token 并重放
5. 在越权测试面板查看结果
...
└── oxff/top/
    ...
    │   ├── history/
    │   │   ├── HistoryPanel.java       # 历史记录面板（搜索/过滤）
    │   │   ├── HistoryContextMenu.java # 历史记录右键菜单
...
| SnakeYAML | 2.2 | `org.yaml:snakeyaml` | YAML 序列化（API 提取规则、判断规则） |
...
构建产物：
- 开发版本: `target/repeater-manager-2.2.0.jar`
- 带时间戳发布版本: `target/releases/repeater-manager-2.2.0-YYYYMMDD-HHMMSS.jar`
