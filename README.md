...
| 令牌方案管理 | 多方案管理令牌位置组合，支持方案-会话关联和全局持久化 |
| 规则组判决机制 | 单活跃规则组 + AND/OR/NOT 多条件组合判决，替代旧版优先级迭代模式 |
| 匿名用户创建 | 一键创建所有令牌值为空的访客用户，智能令牌方案匹配 |
...
src/main/java/
├── org/oxff/repeater/
│   ├── RepeaterManagerExtension.java    # 插件生命周期入口 (Montoya BurpExtension 接口)
    ├── api/                            # API 提取子系统
    ...
    │   ├── HttpMessageParser.java      # HTTP 消息解析器
    ...
    │   ├── GlobalTokenSchemeManager.java # 全局令牌方案管理器
